# AI risk controls — mental-health

This project declares which AI controls it commits to in `air.yaml`, and
`pf air gate hospital mental-health` blocks the merge when one of them is not
...
| `pf air baseline hospital mental-health --suggest` | the controls that already pass |
| `pf air coverage hospital mental-health` | which of them this project enforces |
| `pf air gaps` | only the ones it does not |
| `pf air crosswalk eu-ai-act` | the regulator's view of the same facts |
| `pf air register hospital mental-health` | regenerate `governance/air-register.md` |
