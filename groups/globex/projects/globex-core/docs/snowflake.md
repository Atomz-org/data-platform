...
**globex/globex-core** develops on DuckDB and runs production on
...
is covered, and `pf align validate globex globex-core --stage dialect` is the
...
pf align validate globex globex-core --stage dialect
```
...
```bash
pf capability-add <warehouse> globex globex-core
```
