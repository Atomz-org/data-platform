# Elementary — data observability for acme/acme-eu
...
```bash
pf seed acme acme-eu                     # every build records itself
pf tool elementary run acme acme-eu      # first time: create the tables
pf tool elementary report acme acme-eu   # render the HTML report
pf tool elementary serve acme acme-eu    # read it in a browser
```
