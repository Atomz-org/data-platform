# OpenMetadata — the catalogue for jaffle/jaffle-shop
...
```bash
pf tool openmetadata payload jaffle jaffle-shop   # what would be published
pf tool openmetadata sync jaffle jaffle-shop      # regenerate the artefacts
pf tool openmetadata ingest jaffle jaffle-shop    # run the dbt ingestion
pf tool doctor jaffle jaffle-shop                 # can it actually reach the server
```
...
`tools.yaml`, defaulting to `http://localhost:8585` and `jaffle_jaffle-shop`.
