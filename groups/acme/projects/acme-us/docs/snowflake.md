...
**acme/acme-us** develops on DuckDB and runs production on
...
is covered, and `pf align validate acme acme-us --stage dialect` is the
...
pf align validate acme acme-us --stage dialect
```
...
```bash
pf capability-add <warehouse> acme acme-us
```
