...
flowchart LR
    ...
    subgraph LT["transform · dbt"]
        ...
        T3["data tests<br/>163"]:::model
    end
    subgraph LS["semantics"]
        direction TB
        S1["metrics<br/>32"]:::metric
        S2["dimensions<br/>36"]:::metric
        S3["MDL contract"]:::platform
    end
    subgraph LD["delivery"]
        direction TB
        D1["exposures<br/>35"]:::exposure
...
| data tests | ✓ 163 | `transform/models/_reporting__exposures.yml` | the assumptions the models are allowed to make |
...
| **semantics** | | | |
| metrics | ✓ 32 | `transform/models/semantic/metrics_commodity_prices.yml` | the governed answer to a business question; never ad-hoc SQL |
| dimensions | ✓ 36 | `transform/models/semantic/metrics_commodity_prices.yml` | how a metric may be sliced; conformed ones come from the group |
...
| **delivery** | | | |
| exposures | ✓ 35 | `transform/models/_reporting__exposures.yml` | who reads the output — impact analysis stops at the mart without them |
