...
**`generated/`** — written by `pf evals-gen acme acme-eu`. Each toolkit
...
**Everything else in `evals/cases/`** — yours. Cases that encode acme-eu's own
...
## Commands
    ...
    pf evals-gen acme acme-eu      # (re)ground the toolkit templates
    pf evals acme acme-eu          # contract tier + load every case
    pf evals acme acme-eu --live   # grade against the real models
