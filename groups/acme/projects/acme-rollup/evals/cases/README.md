...
**`generated/`** — written by `pf evals-gen acme acme-rollup`. Each toolkit
...
**Everything else in `evals/cases/`** — yours. Cases that encode acme-rollup's own
...
## Commands
    ...
    pf evals-gen acme acme-rollup      # (re)ground the toolkit templates
    pf evals acme acme-rollup          # contract tier + load every case
    pf evals acme acme-rollup --live   # grade against the real models
