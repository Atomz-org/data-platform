# Recce — dbt review for zenith/zenith-de
...
```bash
pf seed zenith zenith-de                 # build green
pf tool recce baseline zenith zenith-de  # capture that build as the baseline
#   ... change a model ...
pf seed zenith zenith-de
pf tool recce run zenith zenith-de       # diff against the baseline
pf tool recce serve zenith zenith-de     # read it in the Recce UI
```
...
pf artifacts pull zenith zenith-de      # baseline + review onto this machine
pf artifacts ls zenith zenith-de        # what has been published
```
