...
**`generated/`** — written by `pf evals-gen zenith zenith-de`. Each toolkit
...
**Everything else in `evals/cases/`** — yours. Cases that encode zenith-de's own
...
## Commands
    ...
    pf evals-gen zenith zenith-de      # (re)ground the toolkit templates
    pf evals zenith zenith-de          # contract tier + load every case
    pf evals zenith zenith-de --live   # grade against the real models
