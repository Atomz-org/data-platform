---
okf_version: '0.1'
name: commodity/commodity-us
okf_x_group: commodity
okf_x_project: commodity-us
...
Joined to this project's knowledge graph ([kg/graph.json](../kg/graph.json)): every page names the node it documents, and states the lineage, policies and decisions the graph holds for it. The graph reaches 13 model(s); the 7 below are the ones the semantic layer projects to BI.
...
* [fct_us_contract_values_daily](/tables/fct_us_contract_values_daily.md) — `PriceObservation` - The import-parity value of each tracked US exchange contract per its quote basis and per lot — GC per troy ounce and per 100 oz lot, HG per pound and per 25,000 lb lot, ALI per tonne with Section 232 duty grossed on. Computed from the benchmark, not a separate contract quote.
...
* [rpt_us_contract_board](/tables/rpt_us_contract_board.md) — `PriceObservation` - One row per tracked US exchange contract: the latest import-parity value per quote basis and per lot, and how old its benchmark is.
