...
```bash
pf loop list zenith zenith-de          # every loop, born vs earned level
pf loop run-all zenith zenith-de       # run them, refresh STATE.md
pf loop ladder zenith zenith-de        # what blocks the next rung
pf loop proposals list                      # what loops proposed; accept / reject
pf ask zenith zenith-de "revenue by month"   # governed metrics, no SQL
pf logs list zenith zenith-de          # every agent run, traced
```
...
`--notify` posts to the group's channel — `groups/zenith/notify.yaml`, which
