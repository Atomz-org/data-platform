...
- **`TESTING.md`** — `platform/tests/README.md` is generated by
  ...
  would.
- **`PATENTS.md`** — vardportal's reserves all patent rights and states that
  nothing is filed. This repo is Apache 2.0, whose §3 *grants* a patent licence
  from contributors; the two positions are opposites, and which one is wanted
  is a business decision, not a file to copy.
- **`COMMERCIAL.md` — now written**, but not as vardportal's. Theirs says
  commercial use requires a separate written licence, because it is PolyForm
  Noncommercial. This repo is Apache 2.0, so commercial use is *already*
  granted and that sentence would have contradicted `LICENSE`. What the file
  does instead is read the real posture back: what Apache 2.0 grants (§3
  patent) and withholds (§6 trademarks, §7 warranty), and the three pinned
  upstreams that actually constrain shipping — `dlthub-ai-workbench`
  (proprietary, scope limited to dltHub Services), `asqav-compliance`
  (Elastic-2.0, forbids offering it as a hosted service to tenants) and
  `okf-weaver` (no licence declared, so all rights reserved). Every one of
  those is quoted from `pf vendor licences`, which already had the analysis.
