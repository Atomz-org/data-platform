...
**`generated/`** — written by `pf evals-gen globex globex-eu`. Each toolkit
...
**Everything else in `evals/cases/`** — yours. Cases that encode globex-eu's own
...
## Commands
    ...
    pf evals-gen globex globex-eu      # (re)ground the toolkit templates
    pf evals globex globex-eu          # contract tier + load every case
    pf evals globex globex-eu --live   # grade against the real models
