...
type: Reference
title: Stack
description: Three products — OpenMetadata, Dagster and recce — on one database, one
  origin
tags:
- control-plane
- stack
status: stable
generated:
  by: process:claude-code/okf-experiment
...
## The catalogue credential

`catalog_sync` publishes the ontology, the glossary and every table to
OpenMetadata, and authenticates as the ingestion bot. OpenMetadata's documented
way to obtain that token is to open the UI, go to Settings → Bots →
ingestion-bot, and paste it somewhere — which is how eight code locations came
to fail with `OPENMETADATA_JWT_TOKEN is not set` against a catalogue in the same
container.

Inside the stack it does not need carrying. The publisher and the server share a
database and a `FERNET_KEY`, and the bot's JWT is an encrypted row in
`user_entity`; the entrypoint decrypts it once at start and exports it, so every
supervised process inherits a credential nobody typed. It is held in the process
environment and written nowhere.

```bash
pf stack token                                  # print it
export OPENMETADATA_JWT_TOKEN="$(pf stack token)"
pf stack status                                 # says where it resolved from
```

`OPENMETADATA_JWT_TOKEN` still wins when it is set — a narrower bot or a remote
catalogue is a deliberate choice, and silently replacing it with whatever this
database holds would publish to the wrong server. Outside the stack the
environment variable remains the only source, which is what keeps a CI runner
from needing database access to publish a glossary.

One trap, upstream: OpenMetadata 1.13.3's Postgres DDL generates the `isbot`
column from `json->>'deleted'`. Filtering on it selects deleted users and finds
no bots, so the bot flag is read out of the JSON instead.
