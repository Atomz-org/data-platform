# The dev database, served — zenith-uk
...
Development builds into one DuckDB file (`data/zenith_uk.duckdb`). Serve it and
...
it over DuckDB's quack protocol instead of opening the file:
    ...
    pf quack serve zenith zenith-uk     # idempotent; prints the endpoint
    pf quack status                          # every project, served or not
    pf quack stop zenith zenith-uk      # hand the file back
...
- **The token stays on the machine.** Fresh per server, in the 0600 state file
  `data/zenith_uk.quack.json` — gitignored, denied to agent edits by
