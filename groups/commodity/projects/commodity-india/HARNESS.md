<!-- GENERATED by `pf harness commodity commodity-india`, and by every `pf bootstrap`. Never hand-edit: the next bootstrap overwrites it, and `pf harness check` fails on the drift in the meantime. -->
# commodity-india — harness

**commodity** group · entity · module `commodity_india` · prod `snowflake` · [the group's harness](../../HARNESS.md) · [the report's harness](reporting/HARNESS.md)
...
`uv run pf work commodity commodity-india` opens a session in this directory. Turn one loads:
...
| path | verdict | by |
|---|---|---|
| `src/commodity_india/sources/gold_api.py` | impact first | impact_required `**/src/*/sources/*.py` |
...
**Travels with the change** (`harness_required`): a run that changes this project must leave this map, the report's and the group's current, or the commit gate refuses it; `pf harness commodity commodity-india` regenerates them.

## 4. Every pull request

`.github/workflows/commodity-india.yml`, composed by `pf bootstrap` from its capabilities' jobs, narrowed by the `changes` filter:
...
Regenerate with `pf harness commodity commodity-india`; `pf harness check` fails when this file and the files it is read from disagree.
