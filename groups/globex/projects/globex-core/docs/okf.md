...
```bash
pf tool okf build globex globex-core     # regenerate from the semantic layer
pf tool okf check globex globex-core     # stale or non-conformant? exits 1
pf tool okf graph globex globex-core     # does every page resolve in kg/graph.json?
pf tool okf weave globex globex-core     # ask the weaver for what the platform lacks
