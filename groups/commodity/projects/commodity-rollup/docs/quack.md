# The dev database, served — commodity-rollup
...
Development builds into one DuckDB file (`data/commodity_rollup.duckdb`). Serve it and
...
it over DuckDB's quack protocol instead of opening the file:
    ...
    pf quack serve commodity commodity-rollup     # idempotent; prints the endpoint
    pf quack status                          # every project, served or not
    pf quack stop commodity commodity-rollup      # hand the file back
...
- **The token stays on the machine.** Fresh per server, in the 0600 state file
  `data/commodity_rollup.quack.json` — gitignored, denied to agent edits by
