...
**acme/acme-eu** develops on DuckDB and runs production on
...
is covered, and `pf align validate acme acme-eu --stage dialect` is the
...
pf align validate acme acme-eu --stage dialect
```
...
```bash
pf capability-add <warehouse> acme acme-eu
```
