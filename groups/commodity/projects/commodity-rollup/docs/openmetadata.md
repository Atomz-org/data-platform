# OpenMetadata — the catalogue for commodity/commodity-rollup
...
```bash
pf tool openmetadata payload commodity commodity-rollup   # what would be published
pf tool openmetadata sync commodity commodity-rollup      # regenerate the artefacts
pf tool openmetadata ingest commodity commodity-rollup    # database, then dbt
pf tool openmetadata pull commodity commodity-rollup      # owners' edits, as a proposal
pf tool doctor commodity commodity-rollup                 # can it actually reach the server
```
...
```bash
pf tool openmetadata pull commodity commodity-rollup
```
...
`tools.yaml`, defaulting to `http://localhost:8585` and `commodity_commodity-rollup`.
