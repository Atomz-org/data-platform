...
```bash
pf tool okf build jaffle jaffle-shop     # regenerate from the semantic layer
pf tool okf check jaffle jaffle-shop     # stale or non-conformant? exits 1
pf tool okf graph jaffle jaffle-shop     # does every page resolve in kg/graph.json?
pf tool okf weave jaffle jaffle-shop     # ask the weaver for what the platform lacks
