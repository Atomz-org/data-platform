...
```bash
pf loop list acme acme-rollup          # every loop, born vs earned level
pf loop run-all acme acme-rollup       # run them, refresh STATE.md
pf loop ladder acme acme-rollup        # what blocks the next rung
pf loop proposals list                      # what loops proposed; accept / reject
pf ask acme acme-rollup "revenue by month"   # governed metrics, no SQL
pf logs list acme acme-rollup          # every agent run, traced
```
