# The dev database, served — mental-health
...
Development builds into one DuckDB file (`data/mental_health.duckdb`). Serve it and
...
it over DuckDB's quack protocol instead of opening the file:
    ...
    pf quack serve hospital mental-health     # idempotent; prints the endpoint
    pf quack status                          # every project, served or not
    pf quack stop hospital mental-health      # hand the file back
...
- **The token stays on the machine.** Fresh per server, in the 0600 state file
  `data/mental_health.quack.json` — gitignored, denied to agent edits by
