...
```bash
pf tool okf build commodity commodity-us     # regenerate from the semantic layer
pf tool okf check commodity commodity-us     # stale or non-conformant? exits 1
pf tool okf graph commodity commodity-us     # does every page resolve in kg/graph.json?
pf tool okf weave commodity commodity-us     # ask the weaver for what the platform lacks
