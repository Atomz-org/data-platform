...
type: Playbook
title: Scaffolding
description: Three commands. The middle one is the point.
tags:
- scaffolding
- runbook
status: stable
...
location · dbt wiring · **architecture map** · conformance check
...
The `architecture map` step writes the companion to it: `kg/architecture.md`,
which is the map of how the project is *built* rather than what data is in it —
and, on a fresh scaffold, a to-do list, because every feature it does not have
yet is a row naming the command that creates it. See
[ARCHITECTURE-MAPS.md](/reference/architecture-maps.md).
