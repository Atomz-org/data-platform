# AI risk controls — acme-rollup

This project declares which AI controls it commits to in `air.yaml`, and
`pf air gate acme acme-rollup` blocks the merge when one of them is not
...
| `pf air baseline acme acme-rollup --suggest` | the controls that already pass |
| `pf air coverage acme acme-rollup` | which of them this project enforces |
| `pf air gaps` | only the ones it does not |
| `pf air crosswalk eu-ai-act` | the regulator's view of the same facts |
| `pf air register acme acme-rollup` | regenerate `governance/air-register.md` |
