# Recce — dbt review for commodity/commodity-rollup
...
```bash
pf seed commodity commodity-rollup                 # build green
pf tool recce baseline commodity commodity-rollup  # capture that build as the baseline
#   ... change a model ...
pf seed commodity commodity-rollup
pf tool recce run commodity commodity-rollup       # diff against the baseline
pf tool recce serve commodity commodity-rollup     # read it in the Recce UI
```
...
pf artifacts pull commodity commodity-rollup      # baseline + review onto this machine
pf artifacts ls commodity commodity-rollup        # what has been published
```
