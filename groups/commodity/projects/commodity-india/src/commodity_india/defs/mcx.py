"""MCX orchestration: one job per commodity, then dbt, then Evidence.
    ...
    mcx_<commodity>_ingest   job ─┬─ commodity_india/mcx_<commodity>   dlt, that commodity only
                                  ...
                                  └─ mcx_report_site                    evidence sources + build
...
def _report_site_asset(wh: Warehouse):
    @asset(
        name="mcx_report_site",
        key_prefix=[PREFIX],
        group_name="reporting",
        pool=wh.writer_pool,
        deps=[AssetKey([PREFIX, m]) for m in REPORT_MODELS] + [AssetKey([PREFIX, "reporting"])],
        kinds={"evidence"},
        tags={"mcx": ""},
        description="Evidence site build: `npm run sources` extracts the marts, `npm run build` "
                    "renders the MCX summary and one page per commodity to reporting/build.",
    )
...
def mcx_definitions() -> Definitions:
    """Commodity assets, table specs, jobs and schedules. Merged over the
    ...
    site = _report_site_asset(wh)
    # Everything downstream of the MCX tables except the OpenMetadata sync, which
    # talks to a server and has its own job and schedule (commodity_india_catalog_sync):
    # a catalogue that is down must not fail a day's market data.
    pipeline = ((AssetSelection.assets(*INGEST_KEYS).downstream()
                 - AssetSelection.assets(AssetKey([PREFIX, "catalog_sync"])))
                | AssetSelection.assets(AssetKey([PREFIX, "mcx_products"]))
                | AssetSelection.assets(site.key))
    jobs, schedules = _commodity_jobs(settings, pipeline)
    transform_job = define_asset_job(
        name=TRANSFORM_JOB,
        selection=pipeline,
        description="Rebuild only: dbt build of every MCX model and its tests, then the "
                    "Evidence site, without loading anything. For a model or page change.",
    )
    return Definitions(
        assets=[*loaders, _landed_tables_asset(wh, commodities), site],
        jobs=[*jobs, transform_job],
        schedules=schedules,
    )
