...
# commodity-india — architecture

**commodity** group · entity · warehouse `snowflake` · module `commodity_india`
...
flowchart LR
    subgraph LI["ingest"]
        direction TB
        I1["dlt sources<br/>gold_api, mcx, yahoo_finance<br/>3"]:::ingress
        I2["pipeline<br/>commodity_india"]:::platform
        I3["raw tables<br/>6"]:::raw
    end
    subgraph LT["transform · dbt"]
        direction TB
        T1["staging<br/>6"]:::model
        T2["marts<br/>16"]:::model
        T3["data tests<br/>243"]:::model
    end
    subgraph LS["semantics"]
        direction TB
        S1["metrics<br/>109"]:::metric
        S2["dimensions<br/>57"]:::metric
        S3["MDL contract"]:::platform
    end
    subgraph LD["delivery"]
        direction TB
        D1["exposures<br/>110"]:::exposure
        D2["Evidence pages<br/>109"]:::exposure
    end
    I1 --> I2 --> I3 --> T1 --> T2
    T2 --> S1 --> D1 --> D2
    T2 --> S2
...
flowchart TB
    ...
    subgraph LG["what it stops"]
        ...
        E4["CI<br/>commodity-india.yml"]:::good
    end
    A --> O --> P
    A --> K
    P -.governs.-> K
...
| raw tables | ✓ 6 | `data/*.duckdb` | what the pipeline actually landed |
| **transform** | | | |
...
| staging models | ✓ 6 | `transform/models/staging/gold_api/stg_gold_api__spot_prices.sql` | one per raw table, generated from the annotations |
| marts | ✓ 16 | `transform/models/marts/core/dim_commodities.sql` | business entities at a declared grain — what metrics are built on |
| data tests | ✓ 243 | `transform/models/_reporting__exposures.yml` | the assumptions the models are allowed to make |
| project macros | ✓ | `transform/macros/mcx_oi_buildup.sql` | project-local SQL; dialect macros come from the platform toolkits |
...
| **semantics** | | | |
| metrics | ✓ 109 | `transform/models/semantic/metrics_commodity_prices.yml` | the governed answer to a business question; never ad-hoc SQL |
| dimensions | ✓ 57 | `transform/models/semantic/metrics_commodity_prices.yml` | how a metric may be sliced; conformed ones come from the group |
...
| decision records | ✓ 7 | `decisions/ADR-0001-duty-rates-are-back-applied-and-flagged.md` | why this project is shaped the way it is, where code cannot say so |
...
| **delivery** | | | |
| exposures | ✓ 110 | `transform/models/_reporting__exposures.yml` | who reads the output — impact analysis stops at the mart without them |
| Evidence BI | ✓ | `reporting/pages/buy-sell-signals.md` | dashboards as a projection of the metrics, never restating logic |
...
| project unit tests | ✓ | `tests/test_mcx_feed.py` | the project's own Python tests — its connectors and Dagster wiring, offline |
...
**Capabilities:** `air`, `evidence`, `github`, `governance`, `loops`, `okf`, `openmetadata`, `recce`, `snowflake`, `wren`
...
Query before reading: `kg_search`, `kg_neighbors`, `kg_path`, `impact_analysis`. Regenerate this file with `pf arch commodity commodity-india`.
