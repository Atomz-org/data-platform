# AI risk controls — globex-eu

This project declares which AI controls it commits to in `air.yaml`, and
`pf air gate globex globex-eu` blocks the merge when one of them is not
...
| `pf air baseline globex globex-eu --suggest` | the controls that already pass |
| `pf air coverage globex globex-eu` | which of them this project enforces |
| `pf air gaps` | only the ones it does not |
| `pf air crosswalk eu-ai-act` | the regulator's view of the same facts |
| `pf air register globex globex-eu` | regenerate `governance/air-register.md` |
