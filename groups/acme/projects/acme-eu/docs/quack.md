# The dev database, served — acme-eu
...
Development builds into one DuckDB file (`data/acme_eu.duckdb`). Serve it and
...
it over DuckDB's quack protocol instead of opening the file:
    ...
    pf quack serve acme acme-eu     # idempotent; prints the endpoint
    pf quack status                          # every project, served or not
    pf quack stop acme acme-eu      # hand the file back
...
- **The token stays on the machine.** Fresh per server, in the 0600 state file
  `data/acme_eu.quack.json` — gitignored, denied to agent edits by
