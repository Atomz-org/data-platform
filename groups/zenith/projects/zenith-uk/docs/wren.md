# Wren — conversational analytics for zenith/zenith-uk
...
```bash
pf tool wren workspace zenith zenith-uk       # (re)generate the workspace; --index for memory
pf tool wren check zenith zenith-uk           # is the committed workspace what the manifest projects?
pf tool wren context zenith zenith-uk "<q>"   # rules + remembered questions + schema, for one question
