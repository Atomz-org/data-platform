...
```bash
pf tool openmetadata payload zenith zenith-uk   # what would be published
pf tool openmetadata sync zenith zenith-uk      # regenerate the artefacts
pf tool openmetadata ingest zenith zenith-uk    # run the dbt ingestion
pf tool doctor zenith zenith-uk                 # can it actually reach the server
```
...
`tools.yaml`, defaulting to `http://localhost:8585` and `zenith_zenith-uk`.
