...
def derive_profile(project_profiles: dict[str, Any], schema: str = SCHEMA) -> dict[str, Any]:
...
def profile_block(project_dir: Path, schema: str = SCHEMA) -> str:
    ...
    doc = derive_profile(dbtproject.load_yaml(dbt_dir(project_dir) / "profiles.yml"), schema)
    if not doc:
        return ""
    return (
...
def ensure_package(project_dir: Path, version: list[str] | None = None) -> bool:
    return dbtproject.ensure_package(dbt_dir(project_dir), PACKAGE, version or PACKAGE_VERSION)
...
def ensure_models_config(project_dir: Path, schema: str = SCHEMA) -> bool:
    ...
    if dbtproject.has_project_key(d, "models", "elementary"):
        return False
    return dbtproject.insert_under_key(d / "dbt_project.yml", "models", models_block(schema))
...
def build_models(project_dir: Path) -> dict[str, Any]:
    ...
    proc = dbt(project_dir, "run", "--select", "elementary", duckdb_path=env["PF_DUCKDB_PATH"])
    return {"ok": proc.returncode == 0, "message": (proc.stdout or proc.stderr or "")[-2000:]}
...
def run_report(project_dir: Path, timeout: int = 900) -> dict[str, Any]:
    ...
    env = dbt_env(project_dir)
    try:
        # edr opens the dev database through dbt's adapter — a direct file
        # open, invisible to a quack server. Borrow the file like any writer.
        with write_window(env.get("PF_DUCKDB_PATH")):
            proc = subprocess.run(args, cwd=str(d), env=env, capture_output=True, text=True, timeout=timeout)
    except FileNotFoundError:
        return {
            "ok": False,
            "reason": "not_installed",
            "message": "edr is not on PATH — `uv tool install elementary-data==0.25.* --with dbt-duckdb`",
        }
...
def bootstrap_project(root: Path, group: str, project: str, project_dir: Path, config: dict[str, Any]) -> Any:
    ...
    changed = [
        what
        for what, did in (("packages.yml", added_pkg), ("dbt_project.yml", added_cfg), ("profiles.yml", added_profile))
        if did
    ]
...
def dagster_assets(ctx: ToolContext) -> ToolContribution:
    ...
    def _marts() -> list[str]:
        ...
        return sorted(
            ...
            if n.get("resource_type") == "model" and (n.get("path") or "").split("/")[0] == "marts"
        )
...
TOOL = Tool(
    ...
    requires=(Requirement("binary", "edr", "uv tool install elementary-data==0.25.* --with dbt-duckdb"),),
    ...
    surface=Surface(
        ...
        path=f"/{REPORT_FILE}",
        embeddable=True,
        start=("python", "-m", "http.server", "{port}", "--directory", "{dbt_dir}/" + EDR_TARGET),
    ),
    ...
    stack_layer={
        "layer": "observability",
        "title": "Observability (Elementary)",
        "upstream": "elementary",
        "toolkits": ["elementary-observe"],
        "artefacts": f"transform/{EDR_TARGET}/{REPORT_FILE}",
        "node_kinds": [],
    },
)
...
def register_commands(app: Any) -> None:
    ...
    def cmd_run(group: str, project: str) -> None:
        ...
        console.print(f"{mark} elementary models {'built' if result['ok'] else 'failed'}")
        if not result["ok"]:
    ...
    def cmd_serve(
        group: str, project: str, port: int = typer.Option(DEFAULT_PORT), host: str = typer.Option("127.0.0.1")
    ) -> None:
        ...
        if not report_file(d).exists():
            console.print(f"[yellow]![/] no report yet — `pf tool elementary report {group} {project}` first")
            raise typer.Exit(1)
        console.print(f"[green]→[/] http://{host}:{port}/{REPORT_FILE}")
        os.execv(
            sys.executable,
            [sys.executable, "-m", "http.server", str(port), "--bind", host, "--directory", str(report_dir(d))],
        )
