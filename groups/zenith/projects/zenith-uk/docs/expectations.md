# dbt-expectations — zenith/zenith-uk
...
`pf tool expectations config zenith zenith-uk`) regenerates them.

## Running

They are ordinary dbt tests: `pf seed`, `dbt build` and the Dagster assets run
them; `pf tool expectations run zenith zenith-uk` runs just the generated
