...
def _print_run(run, loop: str, group: str, project: str) -> None:
    colour = {"ok": "yellow", "noop": "green", "proposed": "cyan", "circuit_open": "red",
              "error": "red", "escalated": "red", "gate_blocked": "red",
              "paused": "yellow"}.get(run.outcome, "white")
...
def cmd_loop_list(
    ...
    t = Table("loop", "born", "earned", "cadence", "budget", "writes", "source", "waivers",
...
def cmd_loop_status(limit: int = 15,
    ...
    for e in entries:
        t.add_row(e["started_at"][:19], e.get("group", "-"), e["loop"], e["project"],
                  e.get("level", ""), e["outcome"], str(len(e.get("findings") or [])),
...
def cmd_proposals_accept(proposal: str, note: str = typer.Option(""),
    ...
    g, pj = _owner_of(d)
    Ledger(r, g).append(LoopRun(run_id=str(uuid.uuid4())[:8], loop=d["proposal"]["loop"],
