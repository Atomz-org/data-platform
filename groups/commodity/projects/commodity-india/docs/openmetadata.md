# OpenMetadata — the catalogue for commodity/commodity-india
...
```bash
pf tool openmetadata payload commodity commodity-india   # what would be published
pf tool openmetadata sync commodity commodity-india      # regenerate the artefacts
pf tool openmetadata ingest commodity commodity-india    # database, then dbt
pf tool openmetadata pull commodity commodity-india      # owners' edits, as a proposal
pf tool doctor commodity commodity-india                 # can it actually reach the server
```
...
```bash
pf tool openmetadata pull commodity commodity-india
```
...
`tools.yaml`, defaulting to `http://localhost:8585` and `commodity_commodity-india`.
