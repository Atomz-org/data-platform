# AI risk controls — commodity-us

This project declares which AI controls it commits to in `air.yaml`, and
`pf air gate commodity commodity-us` blocks the merge when one of them is not
...
| `pf air baseline commodity commodity-us --suggest` | the controls that already pass |
| `pf air coverage commodity commodity-us` | which of them this project enforces |
| `pf air gaps` | only the ones it does not |
| `pf air crosswalk eu-ai-act` | the regulator's view of the same facts |
| `pf air register commodity commodity-us` | regenerate `governance/air-register.md` |
