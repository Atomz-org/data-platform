# AI risk controls — zenith-uk

This project declares which AI controls it commits to in `air.yaml`, and
`pf air gate zenith zenith-uk` blocks the merge when one of them is not
...
| `pf air baseline zenith zenith-uk --suggest` | the controls that already pass |
| `pf air coverage zenith zenith-uk` | which of them this project enforces |
| `pf air gaps` | only the ones it does not |
| `pf air crosswalk eu-ai-act` | the regulator's view of the same facts |
| `pf air register zenith zenith-uk` | regenerate `governance/air-register.md` |
