...
**hospital/mental-health** develops on DuckDB and runs production on
...
is covered, and `pf align validate hospital mental-health --stage dialect` is the
...
pf align validate hospital mental-health --stage dialect
```
...
```bash
pf capability-add <warehouse> hospital mental-health
```
