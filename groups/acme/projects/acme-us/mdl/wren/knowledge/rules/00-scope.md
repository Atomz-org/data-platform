# Scope — groups/acme/projects/acme-us

This workspace is one project of the `acme` group. Every model in it is a
mart of `acme-us` (MDL catalog `acme`, schema `acme_us`),
...
- A governed number is its metric: answer it from the cube `acme_us_core` or from `query_metrics`, never by recomputing it in SQL.
