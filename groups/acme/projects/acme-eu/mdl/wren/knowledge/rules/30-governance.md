...
| `gdpr-erasure-path` | error | A subject access or erasure request must have a declared path to every copy of the subject's data — marts and review artefacts included, not only the raw tables |
...
| `mart-declares-grain` | error | Grain is the one fact a downstream consumer cannot infer and cannot survive guessing. An undeclared grain is how a join silently fans out. |
