# The dev database, served — globex-eu
...
Development builds into one DuckDB file (`data/globex_eu.duckdb`). Serve it and
...
it over DuckDB's quack protocol instead of opening the file:
    ...
    pf quack serve globex globex-eu     # idempotent; prints the endpoint
    pf quack status                          # every project, served or not
    pf quack stop globex globex-eu      # hand the file back
...
- **The token stays on the machine.** Fresh per server, in the 0600 state file
  `data/globex_eu.quack.json` — gitignored, denied to agent edits by
