...
```bash
pf tool okf build zenith zenith-de     # regenerate from the semantic layer
pf tool okf check zenith zenith-de     # stale or non-conformant? exits 1
pf tool okf graph zenith zenith-de     # does every page resolve in kg/graph.json?
pf tool okf weave zenith zenith-de     # ask the weaver for what the platform lacks
