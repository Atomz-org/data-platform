# Loops — commodity/commodity-rollup
...
```bash
pf loop list commodity commodity-rollup          # every loop, born vs earned level
pf loop run-all commodity commodity-rollup       # run them, refresh STATE.md
pf loop ladder commodity commodity-rollup        # what blocks the next rung
pf loop proposals list                      # what loops proposed; accept / reject
pf ask commodity commodity-rollup "revenue by month"   # governed metrics, no SQL
pf logs list commodity commodity-rollup          # every agent run, traced
```
