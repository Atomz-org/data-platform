# Elementary — data observability for acme/acme-rollup
...
```bash
pf seed acme acme-rollup                     # every build records itself
pf tool elementary run acme acme-rollup      # first time: create the tables
pf tool elementary report acme acme-rollup   # render the HTML report
pf tool elementary serve acme acme-rollup    # read it in a browser
```
