# OpenMetadata — the catalogue for acme/acme-us
...
```bash
pf tool openmetadata payload acme acme-us   # what would be published
pf tool openmetadata sync acme acme-us      # regenerate the artefacts
pf tool openmetadata ingest acme acme-us    # run the dbt ingestion
pf tool doctor acme acme-us                 # can it actually reach the server
```
...
`tools.yaml`, defaulting to `http://localhost:8585` and `acme_acme-us`.
