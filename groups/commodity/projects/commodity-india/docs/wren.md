# Wren — conversational analytics for commodity/commodity-india
...
```bash
pf tool wren workspace commodity commodity-india       # (re)generate the workspace; --index for memory
pf tool wren check commodity commodity-india           # is the committed workspace what the manifest projects?
pf tool wren context commodity commodity-india "<q>"   # rules + remembered questions + schema, for one question
