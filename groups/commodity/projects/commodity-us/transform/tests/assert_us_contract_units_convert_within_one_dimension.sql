-- A lot in bushels priced from a per-pound landed price is only meaningful when
-- both units measure mass. Every US contract quote and lot unit must share a
-- dimension with the commodity's US market unit.
select l.contract_code, mu.market_unit, l.quote_unit, l.lot_unit
from {{ ref('us_contract_lots') }} as l
