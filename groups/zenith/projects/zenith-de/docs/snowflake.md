...
**zenith/zenith-de** develops on DuckDB and runs production on
...
is covered, and `pf align validate zenith zenith-de --stage dialect` is the
...
pf align validate zenith zenith-de --stage dialect
```
...
```bash
pf capability-add <warehouse> zenith zenith-de
```
