...
```bash
pf tool openmetadata payload acme acme-eu   # what would be published
pf tool openmetadata sync acme acme-eu      # regenerate the artefacts
pf tool openmetadata ingest acme acme-eu    # run the dbt ingestion
pf tool doctor acme acme-eu                 # can it actually reach the server
```
...
`tools.yaml`, defaulting to `http://localhost:8585` and `acme_acme-eu`.
