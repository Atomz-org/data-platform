...
**commodity/commodity-us** develops on DuckDB and runs production on
...
is covered, and `pf align validate commodity commodity-us --stage dialect` is the
...
pf align validate commodity commodity-us --stage dialect
```
...
```bash
pf capability-add <warehouse> commodity commodity-us
```
