# OpenMetadata — the catalogue for globex/globex-eu
...
```bash
pf tool openmetadata payload globex globex-eu   # what would be published
pf tool openmetadata sync globex globex-eu      # regenerate the artefacts
pf tool openmetadata ingest globex globex-eu    # run the dbt ingestion
pf tool doctor globex globex-eu                 # can it actually reach the server
```
...
`tools.yaml`, defaulting to `http://localhost:8585` and `globex_globex-eu`.
