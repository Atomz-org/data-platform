# AI risk controls — jaffle-shop

This project declares which AI controls it commits to in `air.yaml`, and
`pf air gate jaffle jaffle-shop` blocks the merge when one of them is not
...
| `pf air baseline jaffle jaffle-shop --suggest` | the controls that already pass |
| `pf air coverage jaffle jaffle-shop` | which of them this project enforces |
| `pf air gaps` | only the ones it does not |
| `pf air crosswalk eu-ai-act` | the regulator's view of the same facts |
| `pf air register jaffle jaffle-shop` | regenerate `governance/air-register.md` |
