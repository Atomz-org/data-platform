...
```bash
pf tool okf build acme acme-us     # regenerate from the semantic layer
pf tool okf check acme acme-us     # stale or non-conformant? exits 1
pf tool okf graph acme acme-us     # does every page resolve in kg/graph.json?
pf tool okf weave acme acme-us     # ask the weaver for what the platform lacks
