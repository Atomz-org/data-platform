...
**commodity/commodity-rollup** develops on DuckDB and runs production on
...
is covered, and `pf align validate commodity commodity-rollup --stage dialect` is the
...
pf align validate commodity commodity-rollup --stage dialect
```
...
```bash
pf capability-add <warehouse> commodity commodity-rollup
```
