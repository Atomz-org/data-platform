# Evals for commodity-india

Prompts are code; these are their tests.

## Two kinds of case live here

**`generated/`** — written by `pf evals-gen commodity commodity-india`. Each toolkit
...
**Everything else in `evals/cases/`** — yours. Cases that encode commodity-india's own
...
## Commands
    ...
    pf evals-gen commodity commodity-india      # (re)ground the toolkit templates
    pf evals commodity commodity-india          # contract tier + load every case
    pf evals commodity commodity-india --live   # grade against the real models
