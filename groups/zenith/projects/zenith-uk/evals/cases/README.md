...
**`generated/`** — written by `pf evals-gen zenith zenith-uk`. Each toolkit
...
**Everything else in `evals/cases/`** — yours. Cases that encode zenith-uk's own
...
## Commands
    ...
    pf evals-gen zenith zenith-uk      # (re)ground the toolkit templates
    pf evals zenith zenith-uk          # contract tier + load every case
    pf evals zenith zenith-uk --live   # grade against the real models
