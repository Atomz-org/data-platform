# OpenMetadata — the catalogue for commodity/commodity-us
...
```bash
pf tool openmetadata payload commodity commodity-us   # what would be published
pf tool openmetadata sync commodity commodity-us      # regenerate the artefacts
pf tool openmetadata ingest commodity commodity-us    # database, then dbt
pf tool openmetadata pull commodity commodity-us      # owners' edits, as a proposal
pf tool doctor commodity commodity-us                 # can it actually reach the server
```
...
```bash
pf tool openmetadata pull commodity commodity-us
```
...
`tools.yaml`, defaulting to `http://localhost:8585` and `commodity_commodity-us`.
