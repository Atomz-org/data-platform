...
`landed_price_local` is the benchmark grossed up by the US duty in force — import parity, since this market prices in USD and needs no
conversion. It is the benchmark itself for a duty-free commodity, and it inherits the
...
<DataTable data={board} rows=20 search=true>
    ...
    <Column id=usd_fx_rate title='USD/USD' fmt=num2/>
    <Column id=effective_duty_rate title='Duty' fmt=pct1/>
    <Column id=landed_price_local title='Import parity (USD)' fmt=num2/>
