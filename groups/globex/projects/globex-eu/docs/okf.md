...
```bash
pf tool okf build globex globex-eu     # regenerate from the semantic layer
pf tool okf check globex globex-eu     # stale or non-conformant? exits 1
pf tool okf graph globex globex-eu     # does every page resolve in kg/graph.json?
pf tool okf weave globex globex-eu     # ask the weaver for what the platform lacks
