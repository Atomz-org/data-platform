...
# acme-rollup — architecture

**acme** group · roll-up · warehouse `snowflake` · module `acme_rollup` · sisters attached: acme-eu, acme-us
...
flowchart LR
    subgraph LI["ingest"]
        ...
        I2["pipeline<br/>acme_rollup"]:::platform
    ...
    S1 -.published as.-> S3
    T2 -.asserted by.-> T3
    RU["sisters attached READ_ONLY<br/>acme-eu, acme-us"]:::project
    RU --> T1
...
| CI workflow | ✓ | `.github/workflows/acme-rollup.yml` | one workflow per project, composed from its capabilities' jobs |
...
Query before reading: `kg_search`, `kg_neighbors`, `kg_path`, `impact_analysis`. Regenerate this file with `pf arch acme acme-rollup`.
