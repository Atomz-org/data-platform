...
New here? [docs/ONBOARDING.md](docs/ONBOARDING.md) is the whole repository as a
newcomer needs it — the tiers, one company's data end to end, and the command
sequence that brings a new company in. Generated by `pf guide build` and
checked on every pull request, so it cannot go stale.
...
| `.memory/notes/` — one lesson per file, for the next agent of any tool (`pf memory add`) | `.memory/MEMORY.md` — the index, generated by `pf memory index` |

Every coding agent — Claude Code, Copilot, Gemini, Codex, a local model — works
to one protocol, [AGENTS.md](AGENTS.md), whose rules go by how the agent was
invoked (session, autonomous, inline) rather than by vendor.
