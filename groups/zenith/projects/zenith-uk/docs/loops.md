...
```bash
pf loop list zenith zenith-uk          # every loop, born vs earned level
pf loop run-all zenith zenith-uk       # run them, refresh STATE.md
pf loop ladder zenith zenith-uk        # what blocks the next rung
pf loop proposals list                      # what loops proposed; accept / reject
pf ask zenith zenith-uk "revenue by month"   # governed metrics, no SQL
pf logs list zenith zenith-uk          # every agent run, traced
```
