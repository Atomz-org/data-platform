...
**commodity/commodity-india** develops on DuckDB and runs production on
...
is covered, and `pf align validate commodity commodity-india --stage dialect` is the
...
```bash
uv sync --extra snowflake    # installs dbt-snowflake
DBT_TARGET=prod dbt build          # explicit, every time
pf align validate commodity commodity-india --stage dialect
```
...
```bash
pf capability-add <warehouse> commodity commodity-india
```
