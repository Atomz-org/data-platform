# Loops — commodity/commodity-india
...
```bash
pf loop list commodity commodity-india          # every loop, born vs earned level
pf loop run-all commodity commodity-india       # run them, refresh STATE.md
pf loop ladder commodity commodity-india        # what blocks the next rung
pf loop proposals list                      # what loops proposed; accept / reject
pf ask commodity commodity-india "revenue by month"   # governed metrics, no SQL
pf logs list commodity commodity-india          # every agent run, traced
```
