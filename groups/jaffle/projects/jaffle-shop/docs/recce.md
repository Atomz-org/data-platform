# Recce — dbt review for jaffle/jaffle-shop
...
```bash
pf seed jaffle jaffle-shop                 # build green
pf tool recce baseline jaffle jaffle-shop  # capture that build as the baseline
#   ... change a model ...
pf seed jaffle jaffle-shop
pf tool recce run jaffle jaffle-shop       # diff against the baseline
pf tool recce serve jaffle jaffle-shop     # read it in the Recce UI
```
...
pf artifacts pull jaffle jaffle-shop      # baseline + review onto this machine
pf artifacts ls jaffle jaffle-shop        # what has been published
```
