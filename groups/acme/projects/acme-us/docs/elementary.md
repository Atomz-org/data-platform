# Elementary — data observability for acme/acme-us
...
```bash
pf seed acme acme-us                     # every build records itself
pf tool elementary run acme acme-us      # first time: create the tables
pf tool elementary report acme acme-us   # render the HTML report
pf tool elementary serve acme acme-us    # read it in a browser
```
