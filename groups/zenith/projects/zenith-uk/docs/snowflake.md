...
**zenith/zenith-uk** develops on DuckDB and runs production on
...
is covered, and `pf align validate zenith zenith-uk --stage dialect` is the
...
pf align validate zenith zenith-uk --stage dialect
```
...
```bash
pf capability-add <warehouse> zenith zenith-uk
```
