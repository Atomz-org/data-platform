# dbt-expectations — hospital/mental-health
...
`pf tool expectations config hospital mental-health`) regenerates them.

## Running

They are ordinary dbt tests: `pf seed`, `dbt build` and the Dagster assets run
them; `pf tool expectations run hospital mental-health` runs just the generated
