...
# acme-eu — architecture

**acme** group · entity · warehouse `snowflake` · module `acme_eu`
...
flowchart LR
    subgraph LI["ingest"]
        ...
        I2["pipeline<br/>acme_eu"]:::platform
...
|---|---|---|---|
| **ingest** | | | |
| dlt sources | ✓ | `src/acme_eu/sources/stripe.py` | the only place raw data enters; each declares its ontology concept |
...
| data tests | **gap** | `transform/models/marts/_core__models.yml` | the assumptions the models are allowed to make — `/add-tests` |
...
| **delivery** | | | |
| exposures | **gap** | `transform/models/marts/_core__models.yml` | who reads the output — impact analysis stops at the mart without them — `/using-dbt` |
...
Query before reading: `kg_search`, `kg_neighbors`, `kg_path`, `impact_analysis`. Regenerate this file with `pf arch acme acme-eu`.
