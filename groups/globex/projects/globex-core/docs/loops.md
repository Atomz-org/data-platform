...
```bash
pf loop list globex globex-core          # every loop, born vs earned level
pf loop run-all globex globex-core       # run them, refresh STATE.md
pf loop ladder globex globex-core        # what blocks the next rung
pf loop proposals list                      # what loops proposed; accept / reject
pf ask globex globex-core "revenue by month"   # governed metrics, no SQL
pf logs list globex globex-core          # every agent run, traced
```
