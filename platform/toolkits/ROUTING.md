...
Four upstream skill sets overlap here. Without this order an agent answers a
governed metric question with ad-hoc SQL.

1. **Structure / lineage** ("where does X come from", "what uses Y", "what
   breaks if I change Z") → `kg_search`, `kg_neighbors`, `kg_path`,
...
- Before any schema or model change, run `impact_analysis` and report the blast
  radius, exposure owners included.
- After changing a model, confirm the blast radius → `recce-review`: lineage
  says what could break, only a diff says what did.
- A failing test or fired monitor → `elementary-observe: triage-observability`.
...
- Every data-returning tool is truncated by policy (schema + <=20 rows + counts).
  Need more? Aggregate in SQL — do not raise the limit.
- Any chart or diagram → `viz-standards`. Form first, colour last; status
