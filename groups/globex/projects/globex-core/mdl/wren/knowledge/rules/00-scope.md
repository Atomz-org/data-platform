# Scope — groups/globex/projects/globex-core

This workspace is one project of the `globex` group. Every model in it is a
mart of `globex-core` (MDL catalog `globex`, schema `globex_core`),
...
- A governed number is its metric: answer it from the cube or from `query_metrics`, never by recomputing it in SQL.
