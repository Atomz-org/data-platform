# The dev database, served — zenith-de
...
Development builds into one DuckDB file (`data/zenith_de.duckdb`). Serve it and
...
it over DuckDB's quack protocol instead of opening the file:
    ...
    pf quack serve zenith zenith-de     # idempotent; prints the endpoint
    pf quack status                          # every project, served or not
    pf quack stop zenith zenith-de      # hand the file back
...
- **The token stays on the machine.** Fresh per server, in the 0600 state file
  `data/zenith_de.quack.json` — gitignored, denied to agent edits by
