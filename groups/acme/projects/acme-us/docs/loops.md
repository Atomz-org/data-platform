...
```bash
pf loop list acme acme-us          # every loop, born vs earned level
pf loop run-all acme acme-us       # run them, refresh STATE.md
pf loop ladder acme acme-us        # what blocks the next rung
pf loop proposals list                      # what loops proposed; accept / reject
pf ask acme acme-us "revenue by month"   # governed metrics, no SQL
pf logs list acme acme-us          # every agent run, traced
```
