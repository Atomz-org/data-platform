# Recce — dbt review for acme/acme-eu
...
```bash
pf seed acme acme-eu                 # build green
pf tool recce baseline acme acme-eu  # capture that build as the baseline
#   ... change a model ...
pf seed acme acme-eu
pf tool recce run acme acme-eu       # diff against the baseline
pf tool recce serve acme acme-eu     # read it in the Recce UI
```
...
pf artifacts pull acme acme-eu      # baseline + review onto this machine
pf artifacts ls acme acme-eu        # what has been published
```
