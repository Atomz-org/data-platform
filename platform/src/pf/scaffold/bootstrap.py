    ...
    steps:
      ...
      - uses: astral-sh/setup-uv@v5
        with:
          enable-cache: true
      - run: uv sync
      # The suite's currency tests compare generated files with the tree; the
      # files themselves are healed by `agent-context`, not failed on, so the
      # suite judges a regenerated tree as `agent-context` does (see there).
      - name: Regenerate generated context
        run: uv run pf context refresh
  ...
  converged:
    ...
    steps:
      ...
      - name: The generated tree matches the committed one
        ...
        #
        # `kg/architecture.md` and the generated expectations tests join them,
        # measured rather than assumed: the same commit rendered in a bare clone
        # and on a checkout with a warehouse produced different maps for four of
        # nine projects. Metrics and dimensions come from
        # `target/semantic_manifest.json`, which a real dbt build writes and
        # `dbt parse` on a bare runner does not; the Evidence page count and the
        # printed location of `data tests` come from generated files that are
        # never committed. Building the graph first narrows the gap and does not
        # close it, so the map is checked where it can be: the per-project
