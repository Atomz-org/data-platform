# The dev database, served — commodity-india
...
Development builds into one DuckDB file (`data/commodity_india.duckdb`). Serve it and
...
it over DuckDB's quack protocol instead of opening the file:
    ...
    pf quack serve commodity commodity-india     # idempotent; prints the endpoint
    pf quack status                          # every project, served or not
    pf quack stop commodity commodity-india      # hand the file back
...
- **The token stays on the machine.** Fresh per server, in the 0600 state file
  `data/commodity_india.quack.json` — gitignored, denied to agent edits by
