# Wren — conversational analytics for acme/acme-eu
...
```bash
pf tool wren workspace acme acme-eu       # (re)generate the workspace; --index for memory
pf tool wren check acme acme-eu           # is the committed workspace what the manifest projects?
pf tool wren context acme acme-eu "<q>"   # rules + remembered questions + schema, for one question
