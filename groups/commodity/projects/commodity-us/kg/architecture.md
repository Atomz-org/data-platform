<!-- GENERATED by `pf arch <group> <project>`, and by every `pf bootstrap`. Never hand-edit: the next bootstrap overwrites it, and `pf arch --check` fails on the drift in the meantime. -->
# commodity-us — architecture

**commodity** group · entity · warehouse `duckdb` · module `commodity_us`

Generated from this project alone. Sister projects are never read; cross-entity questions belong in the roll-up.

## The spine

```mermaid
flowchart LR
    subgraph LI["ingest"]
        direction TB
        I1["dlt sources<br/>gold_api, yahoo_finance<br/>2"]:::ingress
        I2["pipeline<br/>commodity_us"]:::platform
        I3["raw tables<br/>3"]:::raw
    end
    subgraph LT["transform · dbt"]
        direction TB
        T1["staging<br/>3"]:::model
        T2["marts<br/>7"]:::model
        T3["data tests<br/>137"]:::model
    end
    subgraph LS["semantics"]
        direction TB
        S1["metrics<br/>15"]:::metric
        S2["dimensions<br/>23"]:::metric
        S3["MDL contract"]:::platform
    end
    subgraph LD["delivery"]
        direction TB
        D1["exposures<br/>17"]:::exposure
        D2["Evidence pages<br/>17"]:::exposure
    end
    I1 --> I2 --> I3 --> T1 --> T2
    T2 --> S1 --> D1 --> D2
    T2 --> S2
    S1 -.published as.-> S3
    T2 -.asserted by.-> T3
    classDef ingress fill:#cde2fb,stroke:#2a78d6,stroke-width:2px,color:#0b0b0b
    classDef platform fill:#ded9f7,stroke:#4a3aa7,stroke-width:2px,color:#0b0b0b
    classDef raw fill:#fbe8bd,stroke:#eda100,stroke-width:2px,color:#0b0b0b
    classDef model fill:#d6f2e6,stroke:#1baf7a,stroke-width:2px,color:#0b0b0b
    classDef metric fill:#fadfe8,stroke:#e87ba4,stroke-width:2px,color:#0b0b0b
    classDef exposure fill:#fbdccf,stroke:#eb6834,stroke-width:2px,color:#0b0b0b
    classDef neutral fill:#e8e7e2,stroke:#898781,stroke-width:2px,color:#0b0b0b
    classDef project fill:#e3eefc,stroke:#2a78d6,stroke-width:2px,color:#0b0b0b
    classDef lane fill:#f6f6f4,stroke:#898781,stroke-width:2px,color:#0b0b0b
    class LI,LT,LS,LD lane
```

## A real path through it

```mermaid
flowchart LR
    X0["Table<br/>futures_prices"]:::raw
    X1["Model<br/>stg_yahoo_finance__futures_prices"]:::model
    X2["Model<br/>int_commodity_prices__usd"]:::model
    X3["Model<br/>fct_commodity_prices_daily"]:::model
    X4["Exposure<br/>report_import_parity"]:::exposure
    X0 --> X1 --> X2 --> X3 --> X4
    classDef exposure fill:#fbdccf,stroke:#eb6834,stroke-width:2px,color:#0b0b0b
    classDef model fill:#d6f2e6,stroke:#1baf7a,stroke-width:2px,color:#0b0b0b
    classDef raw fill:#fbe8bd,stroke:#eda100,stroke-width:2px,color:#0b0b0b
```

## What enforces what

```mermaid
flowchart TB
    A["annotations<br/>contracts/annotations.yaml<br/>1"]:::platform
    O["ontology<br/>platform + group + project"]:::platform
    P["policy rules<br/>32"]:::platform
    K["knowledge graph<br/>kg_search · impact_analysis"]:::platform
    subgraph LG["what it stops"]
        direction TB
        E1["pf check<br/>conformance"]:::good
        E2["PreToolUse hook<br/>denies secrets + generated files"]:::good
        E3["pre-commit<br/>blast radius"]:::good
        E4["CI<br/>commodity-us.yml"]:::good
    end
    A --> O --> P
    A --> K
    P -.governs.-> K
    K --> E2
    K --> E3
    K --> E4
    O --> E1
    classDef platform fill:#ded9f7,stroke:#4a3aa7,stroke-width:2px,color:#0b0b0b
    classDef good fill:#d5f2d5,stroke:#0ca30c,stroke-width:2px,color:#0b0b0b
    classDef neutral fill:#e8e7e2,stroke:#898781,stroke-width:2px,color:#0b0b0b
    classDef lane fill:#f6f6f4,stroke:#898781,stroke-width:2px,color:#0b0b0b
    class LG lane
```

## Every feature, present or not

| feature | | where | what it is |
|---|---|---|---|
| **ingest** | | | |
| dlt sources | ✓ 2 | `src/commodity_us/sources/gold_api.py` | the only place raw data enters; each declares its ontology concept |
| dlt config | ✓ | `.dlt/config.toml` | pipeline settings and destination; secrets live beside it, ungitted |
| ontology annotations | ✓ | `contracts/annotations.yaml` | concept and column roles — drives staging, PII policy and monitors |
| raw tables | ✓ 3 | `data/*.duckdb` | what the pipeline actually landed |
| **transform** | | | |
| dbt project | ✓ | `transform/dbt_project.yml` | the transform runtime; macro-paths reach the platform's macros |
| dbt targets | ✓ | `transform/profiles.yml` | dev, base and prod; base is what Recce diffs against |
| staging models | ✓ 3 | `transform/models/staging/gold_api/stg_gold_api__spot_prices.sql` | one per raw table, generated from the annotations |
| marts | ✓ 7 | `transform/models/marts/core/dim_commodities.sql` | business entities at a declared grain — what metrics are built on |
| data tests | ✓ 137 | `transform/models/_reporting__exposures.yml` | the assumptions the models are allowed to make |
| project macros | — | `transform/macros/**/*.sql` | project-local SQL; dialect macros come from the platform toolkits — `/using-dbt` |
| snapshots | — | `transform/snapshots/**/*.sql` | slowly-changing dimensions captured over time — `/using-dbt` |
| dbt packages | ✓ | `transform/packages.yml` | the group's shared dbt package, plus any upstream ones |
| **semantics** | | | |
| metrics | ✓ 15 | `transform/models/semantic/metrics_commodity_prices.yml` | the governed answer to a business question; never ad-hoc SQL |
| dimensions | ✓ 23 | `transform/models/semantic/metrics_commodity_prices.yml` | how a metric may be sliced; conformed ones come from the group |
| knowledge graph | ✓ | `kg/graph.json` | kg_search, kg_neighbors and impact analysis read this, not the files |
| context card | ⟳ | `kg/context_card.md` | the always-on index; ~400 tokens, budgeted by `pf tokens` — regenerated by `pf kg card`, not tracked |
| architecture map | ✓ | `kg/architecture.md` | this document — the on-demand map, regenerated never hand-edited |
| project atlas | ✓ | `atlas.yaml` | a picture of this project's own graph, refreshed around its dbt runs |
| MDL manifest | ✓ | `mdl/mdl.json` | the semantic contract an external consumer reads (WrenAI, BI) |
| catalog export | ✓ | `catalog/openmetadata.json` | OpenMetadata ingestion, for a company that runs one |
| Wren workspace | ✓ 3 | `mdl/wren` | the semantic layer as an LLM may read it: restricted columns withheld, rules and remembered questions beside it |
| **governance** | | | |
| project policy | ✓ 32 | `governance/policy.yaml` | the third ontology layer; may only tighten platform and group |
| otop manifest | ✓ 15 | `governance/otop.json` | policy and evidence as an OpenTopology graph, schema-validated |
| project rules | ✓ | `CLAUDE.md` | the rules the graph cannot encode; loaded every session |
| agent permissions | ✓ | `.claude/settings.json` | the deny list and the PreToolUse hook — sisters unreadable by policy |
| harness map | ✓ | `HARNESS.md` | what wraps an agent here — settings, gate verdicts, hooks, CI, loops — read from the files that enforce it |
| MCP servers | ✓ | `.mcp.json` | the servers a capability wires into the agent's session; merged, never overwritten |
| decision records | ✓ 2 | `decisions/ADR-0001-us-duty-is-the-published-rate-with-origin-left-out.md` | why this project is shaped the way it is, where code cannot say so |
| requirements | — | `requirements/**/*.md` | the business requirements it was built from: page snapshot, validated spec, trace — `/build-from-requirement` |
| **delivery** | | | |
| exposures | ✓ 17 | `transform/models/_reporting__exposures.yml` | who reads the output — impact analysis stops at the mart without them |
| Evidence BI | ✓ | `reporting/pages/import-parity.md` | dashboards as a projection of the metrics, never restating logic |
| capability docs | ✓ 9 | `docs/air.md` | one page per capability, explaining what it wired in |
| published pages | — | `*.html` | standalone HTML about this project, kept beside what it describes — `written by hand` |
| **operate** | | | |
| Dagster definitions | ✓ | `src/commodity_us/definitions.py` | assets come from the runtime factory; the project supplies logic |
| Dagster registration | ⟳ | `platform/workspace.yaml` | the code location `pf bootstrap` writes into platform/workspace.yaml, machine-local and ungitted — regenerated by `pf bootstrap`, not tracked |
| CI workflow | ✓ | `.github/workflows/commodity-us.yml` | one workflow per project, composed from its capabilities' jobs |
| project unit tests | — | `tests/**/*.py` | the project's own Python tests — its connectors and Dagster wiring, offline — `written with the code they test` |
| eval cases | **gap** | `evals/cases/**/*.yaml` | does an agent still answer this project's questions correctly — `pf evals-gen` |
| agent memory | ✓ | `.memory/notes` | session notes that survive a compaction |
| duckdb memories | ⟳ | `.duckdb-skills/**` | query patterns the duckdb-ops toolkit reuses — regenerated by `pf bootstrap`, not tracked |
| tool overrides | ✓ | `tools.yaml` | opt out of a group tool or retune one; merged over the group's |
| project deps | ✓ | `pyproject.toml` | the project's own Python dependencies, on top of the platform's |
| air | ✓ | `air.yaml` | AI control baseline: declare it in air.yaml, gate the merge on it. |
| okf | ✓ | `okf/index.md` | Open Knowledge Format: the semantic layer as portable, validated context for agents. |

**Capabilities:** `air`, `ducklake`, `evidence`, `github`, `governance`, `loops`, `okf`, `openmetadata`, `recce`, `wren`

**Tools enabled:** `okf`, `openmetadata`, `recce`, `wren`

## Gaps

- **eval cases** — does an agent still answer this project's questions correctly. Fix: `pf evals-gen`

---

Query before reading: `kg_search`, `kg_neighbors`, `kg_path`, `impact_analysis`. Regenerate this file with `pf arch commodity commodity-us`.
