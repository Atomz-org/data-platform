# dbt-expectations — jaffle/jaffle-shop
...
`pf tool expectations config jaffle jaffle-shop`) regenerates them.

## Running

They are ordinary dbt tests: `pf seed`, `dbt build` and the Dagster assets run
them; `pf tool expectations run jaffle jaffle-shop` runs just the generated
