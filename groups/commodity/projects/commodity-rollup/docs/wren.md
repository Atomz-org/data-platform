# Wren — conversational analytics for commodity/commodity-rollup
...
```bash
pf tool wren workspace commodity commodity-rollup       # (re)generate the workspace; --index for memory
pf tool wren check commodity commodity-rollup           # is the committed workspace what the manifest projects?
pf tool wren context commodity commodity-rollup "<q>"   # rules + remembered questions + schema, for one question
