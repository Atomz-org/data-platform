# Wren — semantic layer for acme/acme-rollup
...
```bash
pf tool wren mdl acme acme-rollup          # what the manifest contains
pf tool wren plan acme acme-rollup "<sql>" # expand SQL through the MDL
pf tool wren query acme acme-rollup "<sql>"
