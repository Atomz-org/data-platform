# Evals for commodity-rollup

Prompts are code; these are their tests.

## Two kinds of case live here

**`generated/`** — written by `pf evals-gen commodity commodity-rollup`. Each toolkit
...
**Everything else in `evals/cases/`** — yours. Cases that encode commodity-rollup's own
...
## Commands
    ...
    pf evals-gen commodity commodity-rollup      # (re)ground the toolkit templates
    pf evals commodity commodity-rollup          # contract tier + load every case
    pf evals commodity commodity-rollup --live   # grade against the real models
