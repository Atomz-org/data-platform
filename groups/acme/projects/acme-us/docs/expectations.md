...
`pf tool expectations config acme acme-us`) regenerates them.

## Running

They are ordinary dbt tests: `pf seed`, `dbt build` and the Dagster assets run
them; `pf tool expectations run acme acme-us` runs just the generated
