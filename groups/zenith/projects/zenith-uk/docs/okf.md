...
```bash
pf tool okf build zenith zenith-uk     # regenerate from the semantic layer
pf tool okf check zenith zenith-uk     # stale or non-conformant? exits 1
pf tool okf graph zenith zenith-uk     # does every page resolve in kg/graph.json?
pf tool okf weave zenith zenith-uk     # ask the weaver for what the platform lacks
