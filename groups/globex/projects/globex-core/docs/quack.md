# The dev database, served — globex-core
...
Development builds into one DuckDB file (`data/globex_core.duckdb`). Serve it and
...
it over DuckDB's quack protocol instead of opening the file:
    ...
    pf quack serve globex globex-core     # idempotent; prints the endpoint
    pf quack status                          # every project, served or not
    pf quack stop globex globex-core      # hand the file back
...
- **The token stays on the machine.** Fresh per server, in the 0600 state file
  `data/globex_core.quack.json` — gitignored, denied to agent edits by
