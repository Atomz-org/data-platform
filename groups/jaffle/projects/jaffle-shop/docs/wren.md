# Wren — conversational analytics for jaffle/jaffle-shop
...
```bash
pf tool wren workspace jaffle jaffle-shop       # (re)generate the workspace; --index for memory
pf tool wren check jaffle jaffle-shop           # is the committed workspace what the manifest projects?
pf tool wren context jaffle jaffle-shop "<q>"   # rules + remembered questions + schema, for one question
...
`groups/jaffle/loop-ledger.json` as a `wren-query` run. A statement refused
