...
## ontology-design — A domain the vocabulary does not describe yet: new classes, identities, roles or relations.
  `design-ontology`
  - Which tier first: a word one company invented never goes in the platform ontology.
  - Identity is a decision, not a source column; relations are named with a business verb.
  - A property with no column is a gap to record, not a field to model.
