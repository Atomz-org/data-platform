# OpenMetadata — the catalogue for zenith/zenith-de
...
```bash
pf tool openmetadata payload zenith zenith-de   # what would be published
pf tool openmetadata sync zenith zenith-de      # regenerate the artefacts
pf tool openmetadata ingest zenith zenith-de    # run the dbt ingestion
pf tool doctor zenith zenith-de                 # can it actually reach the server
```
...
`tools.yaml`, defaulting to `http://localhost:8585` and `zenith_zenith-de`.
