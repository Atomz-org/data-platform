# AI risk controls — commodity-india

This project declares which AI controls it commits to in `air.yaml`, and
`pf air gate commodity commodity-india` blocks the merge when one of them is not
...
| `pf air baseline commodity commodity-india --suggest` | the controls that already pass |
| `pf air coverage commodity commodity-india` | which of them this project enforces |
| `pf air gaps` | only the ones it does not |
| `pf air crosswalk eu-ai-act` | the regulator's view of the same facts |
| `pf air register commodity commodity-india` | regenerate `governance/air-register.md` |
