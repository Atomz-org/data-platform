# dbt-expectations — acme/acme-rollup
...
`pf tool expectations config acme acme-rollup`) regenerates them.

## Running

They are ordinary dbt tests: `pf seed`, `dbt build` and the Dagster assets run
them; `pf tool expectations run acme acme-rollup` runs just the generated
